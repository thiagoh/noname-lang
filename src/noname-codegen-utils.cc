...
AllocaInst* alloca_typed_var_codegen(int type, llvm::BasicBlock* bb = nullptr) {
  ...
  if (type == TYPE_DOUBLE) {
    typed_pointer_alloca =
        new AllocaInst(Type::getDoubleTy(TheContext), "alloca_double_v");
    typed_pointer_alloca->setAlignment(8);

  } else if (type == TYPE_FLOAT) {
    typed_pointer_alloca =
        new AllocaInst(Type::getFloatTy(TheContext), "alloca_float_v");
    typed_pointer_alloca->setAlignment(4);

  } else if (type == TYPE_LONG) {
    typed_pointer_alloca =
        new AllocaInst(IntegerType::get(TheContext, 64), "alloca_long_v");
    typed_pointer_alloca->setAlignment(8);

  } else if (type == TYPE_INT) {
    typed_pointer_alloca =
        new AllocaInst(IntegerType::get(TheContext, 32), "alloca_int_v");
    typed_pointer_alloca->setAlignment(4);

  } else if (type == TYPE_SHORT) {
    typed_pointer_alloca =
        new AllocaInst(IntegerType::get(TheContext, 16), "alloca_short_v");
    typed_pointer_alloca->setAlignment(2);

  } else if (type == TYPE_CHAR) {
    typed_pointer_alloca =
        new AllocaInst(IntegerType::get(TheContext, 8), "alloca_char_v");
...
StoreInst* store_typed_var_codegen(int type, Value* value,
...
StoreInst* store_untyped_var_codegen(int type, CastInst* cast_inst_from,
                                     AllocaInst* alloca_inst_to,
                                     llvm::BasicBlock* bb = nullptr) {
  StoreInst* store_inst =
      new StoreInst(cast_inst_from, alloca_inst_to, false, bb);
...
CastInst* cast_codegen(int type, AllocaInst* alloca_inst_from,
                       llvm::BasicBlock* bb = nullptr) {
  CastInst* casted_inst =
      new BitCastInst(alloca_inst_from, PointerTy_4, "cast_inst_");

  return casted_inst;
}

std::vector<std::unique_ptr<Value>> assign_codegen_util(
    AllocaInst* untyped_poiter_alloca, AssignmentNode* assignment,
    llvm::BasicBlock* bb) {
  /**
  ...
  int rhs_type = fromLLVMValueToType(value_codegen);
  ...
  //  -->  StoreInst* void_34 = new StoreInst(value_const, typed_pointer_alloca,
  //  false);
  //  -->  void_34->setAlignment(4);
  StoreInst* store_inst_typed_var =
      store_typed_var_codegen(rhs_type, value_codegen, bb);

  // Cast the the "typed" variable to the "untyped" variable
  //  -->  CastInst* casted_inst = new BitCastInst(typed_pointer_alloca,
  //  PointerTy_4, "");
  ...
  //  -->  StoreInst* voidp_store = new StoreInst(casted_inst,
  //  untyped_poiter_alloca, false);
  //  -->  voidp_store->setAlignment(8);
  StoreInst* store_inst_untyped_var =
      store_untyped_var_codegen(rhs_type, cast_inst, alloca_inst, bb);
